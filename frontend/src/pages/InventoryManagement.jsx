...
import MedicineSearchWithRecents from '../components/MedicineSearchWithRecents';
...
const InventoryManagement = () => {
    ...
    const [stockUpdateData, setStockUpdateData] = useState({ id: null, name: '', newStock: '' });
    ...
    const handleUpdateMedicine = async (e) => {
        ...
        try {
            ...
            const response = await axios.put(`/api/pharmacists/medicines/${editingMedicine}`, medicineData);

            if (response.data.medicine) {
                setMedicines(medicines.map(med =>
                    med._id === editingMedicine ? response.data.medicine : med
                ));
    ...
    const confirmDeleteMedicine = async () => {
        try {
            const response = await axios.delete(`/api/pharmacists/medicines/${medicineToDelete.id}`);

            if (response.data.message) {
                // Remove the medicine from the state
                setMedicines(medicines.filter(med => med._id !== medicineToDelete.id));
                ...
                // If we're editing the deleted medicine, close the edit form
                if (editingMedicine === medicineToDelete.id) {
                    setEditingMedicine(null);
                    setEditFormData({});
                }
            }
            ...
            // Show error message
            showToast(err.response?.data?.message || 'Unable to remove medicine. Please try again or contact support.', 'error');
        }
    };
    ...
    const openUpdateStockModal = (medicine) => {
        setStockUpdateData({
            id: medicine._id,
            name: medicine.name,
    ...
    const closeUpdateStockModal = () => {
        ...
        setStockUpdateData({ id: null, name: '', newStock: '' });
    };

    const confirmUpdateStock = () => {
        if (stockUpdateData.newStock !== '' && !isNaN(stockUpdateData.newStock) && parseInt(stockUpdateData.newStock) >= 0) {
            handleUpdateStock(stockUpdateData.id, stockUpdateData.newStock);
        ...
        <div className="dashboard-content">
                    ...
                    <form onSubmit={handleUpdateMedicine}>
                        <div className="form-grid">
                            ...
                            <div className="form-group">
                                ...
                                <select
                                    className="form-select"
                                    value={editFormData.category || 'Medicines & Treatments'}
                            ...
                            <div className="form-group">
                                ...
                                <select
                                    className="form-select"
                                    value={editFormData.dosageForm || 'tablet'}
                            ...
                            <div className="form-group checkbox-group">
                                <label>
                                    <input
                                        type="checkbox"
                                        checked={editFormData.requiresPrescription || false}
                        ...
                        <div className="form-actions">
                            ...
                            <button
                                ...
                                onClick={() => handleDeleteMedicine(editingMedicine, editFormData.name)}
                            >
                                Remove Medicine
                            </button>
                            <button
            ...
            {showDeleteConfirm && (
                <div className="modal-overlay">
                    <div className="modal-content">
                        ...
                        <p>Are you sure you want to permanently delete "{medicineToDelete?.name}" from the inventory? This action cannot be undone.</p>
            ...
            {showUpdateStockModal && (
                <div className="modal-overlay">
                    <div className="modal-content">
                        <h3>Update Stock</h3>
                        <p>Enter new stock quantity for <strong>{stockUpdateData.name}</strong>:</p>
                        <div className="form-group">
                            <input
                                type="number"
                                className="form-input"
                                value={stockUpdateData.newStock}
                                onChange={(e) => setStockUpdateData({ ...stockUpdateData, newStock: e.target.value })}
                                min="0"
                                autoFocus
                            />
                        </div>
                                        ...
                                        <td>
                                            <div className="stock-info">
                                                ...
                                                <button
                                                    ...
                                                    onClick={() => openUpdateStockModal(medicine)}
                                                >
                                                    Update
                                                </button>
                                            </div>
